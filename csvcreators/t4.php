...
foreach ($data as $value) {
    ...
    foreach ($tempData['images'] as $img) {
        ...
        download_remote_file_with_curl($img,'sngapparelinc',basename($img));

        if(isset($img['small']))
            download_remote_file_with_curl($img['small'],'sngapparelinc',basename($img['small']));
    }
    ...
    $csvData[$count]['image']                     = 'sngapparelinc/'.basename(str_replace(array(",", '\'', '"' ), "", $tempData['images'][0]['large']));
    $csvData[$count]['small_image']               = 'sngapparelinc/'.basename(str_replace(array(",", '\'', '"' ), "", $tempData['images'][0]['small']));
    $csvData[$count]['thumbnail']                 = 'sngapparelinc/'.basename(str_replace(array(",", '\'', '"' ), "", $tempData['images'][0]['small']));
    $csvData[$count]['image_label']               = $tempData['title'];
    $csvData[$count]['small_image_label']         = $tempData['title'];
    $csvData[$count]['thumbnail_label']           = $tempData['title'];
...
function download_remote_file_with_curl($fileUrl,$source,$saveTo)
{
    $ch = curl_init();
    ...
    if(!file_exists('/var/www/wholesaleleggings82/media/catalog/product/'.$source.'/')) {
        mkdir ('/var/www/wholesaleleggings82/media/catalog/product/'.$source.'/',0700, TRUE);
    }

    $downloaded_file = fopen('/var/www/wholesaleleggings82/media/catalog/product/'.$source.'/'.$saveTo, 'w+');
