...
foreach ($data as $value) {
    ...
    foreach ($tempData['images'] as $img) {
        $imgname = basename(str_replace(array(",", '\'', '"' ), "", $img['large']));
        $imgStr .= 'sngapparelinc_'.$imgname.";";

        download_remote_file_with_curl($img['large'],$imgname);

        if(isset($img['small'])) {
            $tempimg = str_replace(array(",", '\'', '"' ), "", $img['small']);
            download_remote_file_with_curl($tempimg,basename($tempimg));
        }
...
function download_remote_file_with_curl($fileUrl,$saveTo)
{
    $ch = curl_init();
